...
public class AuthController {
    ...
    public ResponseEntity<?> signup(@Valid @RequestBody SignUpReqDto signUpReqDto) {
        AppUser user = AppUser.builder()
                ...
                .status(false)
                .role(signUpReqDto.getRole().toUpperCase())
                .build();
...
//                    notifyAdmins(jwtFilter.getCurrentUserEmail(), "approval");
    ...
    @SecurityRequirement(name = "bearerAuth")
     ...
     @Transactional
    public ResponseEntity<?>  deleteAppUser(@PathVariable("id") String id){
        try{
            ...
            if(jwtFilter.isAdmin()) {
                ...
                return new ResponseEntity<>(CommonResponseMessageDto.builder().message("Unauthorized").build(), HttpStatus.NOT_FOUND);
            }
